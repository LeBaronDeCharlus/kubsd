# kubsd-agentd Milestone 4: Reconciliation Core — Implementation Plan
...
**Goal:** Build `kubsd-agentd`'s reconciliation core: a `Reconciler` that
composes `kubsd-jail`, `kubsd-zfs`, and `kubsd-net` to bring real jails
...
milestones. Also fixes a cross-cutting gap: `kubsd-jail` gets a new
`jail_exists` method, since `is_running` alone can't distinguish "jail
doesn't exist" from "jail exists but its process died".

**Architecture:** `kubsd-agentd` is a library crate exposing a generic
...
- Design spec: `docs/superpowers/specs/2026-07-05-kubsd-agent-design.md` (Approved). The `Reconciler` API, `JailRecord` data model, naming/path derivation, and backoff mechanics there must match exactly.
- `kubsd-agentd` is a **library crate only** this milestone — no `main.rs`, no binary target, no HTTP API, no CLI.
...
- Naming/path derivation: jail name `kubsd-<spec-name>`; base dataset `<pool>/kubsd/<image>` (image already includes its full relative path, e.g. `base/14.2-web`); jail dataset `<pool>/kubsd/jails/<spec-name>`; rootfs path `/<jail-dataset-path>`; epair base name `epair<ordinal>`.
...
Modify `kubsd-jail/src/lib.rs` — add `jail_exists` to the `JailRuntime` trait, right after `create`:
...
Modify `kubsd-jail/src/fake.rs` — add this method to the `impl JailRuntime
...
Modify `kubsd-jail/src/process.rs`. Extract the `jls`-based existence
...
Add this test to `kubsd-jail/tests/freebsd_lifecycle.rs`:
...
```bash
git add kubsd-jail/src/lib.rs kubsd-jail/src/fake.rs kubsd-jail/src/process.rs kubsd-jail/tests/freebsd_lifecycle.rs
...
Run: `ssh root@192.168.64.2 'cd ~/kubsd && git pull && cargo test -p kubsd-jail --test freebsd_lifecycle 2>&1 | tail -15'`
...
### Task 2: kubsd-agentd scaffold — JailRecord and naming/path derivation

**Files:**
- Create: `kubsd-agentd/Cargo.toml`
- Create: `kubsd-agentd/src/record.rs`
- Create: `kubsd-agentd/src/lib.rs`
- Modify: `Cargo.toml` (workspace root — add `kubsd-agentd` to `members`)

**Interfaces:**
- Consumes: `kubsd_spec::JailSpec` (Milestone 1).
...
- [ ] **Step 1: Add kubsd-agentd to the workspace**
...
[workspace]
resolver = "2"
members = ["kubsd-spec", "kubsd-jail", "kubsd-zfs", "kubsd-net", "kubsd-agentd"]
```
...
[dependencies]
kubsd-spec = { path = "../kubsd-spec" }
kubsd-jail = { path = "../kubsd-jail" }
kubsd-zfs = { path = "../kubsd-zfs" }
kubsd-net = { path = "../kubsd-net" }
...
mod tests {
    ...
    #[test]
    fn jail_name_adds_kubsd_prefix() {
        assert_eq!(jail_name("web-1"), "kubsd-web-1");
    }

    #[test]
    fn base_dataset_path_appends_image_directly() {
        assert_eq!(base_dataset_path("zroot", "base/14.2-web"), "zroot/kubsd/base/14.2-web");
    }

    #[test]
    fn jail_dataset_path_uses_jails_subdirectory() {
        assert_eq!(jail_dataset_path("zroot", "web-1"), "zroot/kubsd/jails/web-1");
    }

    #[test]
    fn jail_rootfs_path_is_leading_slash_plus_dataset_path() {
        assert_eq!(jail_rootfs_path("zroot", "web-1"), PathBuf::from("/zroot/kubsd/jails/web-1"));
    }
...
Run: `cargo test --workspace -p kubsd-agentd`

Expected: PASS, 6 tests (`jail_name_adds_kubsd_prefix`,
...
```bash
git add Cargo.toml kubsd-agentd/Cargo.toml kubsd-agentd/src/record.rs kubsd-agentd/src/lib.rs
git commit -m "Add kubsd-agentd crate: JailRecord and naming/path derivation"
git push origin master:main
```
...
**Files:**
- Create: `kubsd-agentd/src/store.rs`
- Modify: `kubsd-agentd/src/lib.rs`
...
mod tests {
    ...
    fn sample_spec(name: &str) -> kubsd_spec::JailSpec {
        kubsd_spec::JailSpec {
            api_version: "kubsd/v1".to_string(),
            kind: "Jail".to_string(),
            metadata: Metadata { name: name.to_string() },
    ...
    fn test_state_dir(name: &str) -> PathBuf {
        let dir = std::env::temp_dir().join(format!("kubsd-agentd-store-test-{name}"));
...
```bash
git add kubsd-agentd/src/store.rs kubsd-agentd/src/lib.rs
git commit -m "Add kubsd-agentd state store: save/load_all/remove"
git push origin master:main
```
...
**Files:**
- Create: `kubsd-agentd/src/backoff.rs`
- Modify: `kubsd-agentd/src/lib.rs`
...
```bash
git add kubsd-agentd/src/backoff.rs kubsd-agentd/src/lib.rs
git commit -m "Add kubsd-agentd per-jail backoff state"
git push origin master:main
```
...
**Files:**
- Create: `kubsd-agentd/src/reconciler.rs`
- Modify: `kubsd-agentd/src/lib.rs`

**Interfaces:**
- Consumes: `JailRecord`, naming/path functions (Task 2); `store::{load_all, save, remove}` (Task 3); `kubsd_spec::{JailSpec, validate_name, validate_address, parse_cpu_cores, parse_memory_bytes, validate_transition, SpecError}` (Milestone 1); `JailRuntime`, `ZfsManager`, `NetManager` traits and their `Fake*` implementations (Milestones 2-3).
...
```bash
git add kubsd-agentd/src/reconciler.rs kubsd-agentd/src/lib.rs
...
**Files:**
- Modify: `kubsd-agentd/src/reconciler.rs`
...
Add these three methods to the existing `impl<J: JailRuntime, Z: ZfsManager, N: NetManager> Reconciler<J, Z, N>` block in `kubsd-agentd/src/reconciler.rs`, after `delete`:
...
`kubsd-agentd/src/reconciler.rs`:
...
```bash
git add kubsd-agentd/src/reconciler.rs
...
Add `use std::time::Instant;` to the top of `kubsd-agentd/src/reconciler.rs`'s
...
- `cargo test --workspace` passes with 70 tests on macOS, all of
  `kubsd-agentd`'s reconciliation logic tested against
