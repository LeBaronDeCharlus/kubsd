# keel-agent Milestone 2: keel-jail + keel-zfs Crates — Implementation Plan
...
**Goal:** Build `keel-jail` (jail lifecycle + resource limits via
`jail(8)`/`jexec(8)`/`jls(8)`/`rctl(8)`) and `keel-zfs` (ZFS clone
provisioning via `zfs(8)`) — the two crates that give keel-agentd a real,
testable way to create and tear down non-networked jails from a base ZFS
image. VNET networking is explicitly out of scope (that's `keel-net`, a
later milestone).
...
(only runs on the FreeBSD VM). This mirrors `keel-spec`'s
...
- Design spec: `docs/superpowers/specs/2026-07-05-keel-agent-design.md` (Approved). The `JailRuntime` and `ZfsManager` trait signatures below are copied verbatim from its Architecture section — do not deviate from them.
...
- No VNET/networking in `keel-jail` or `keel-zfs` this milestone.
- `keel-jail` and `keel-zfs` take plain names/paths — neither crate knows about keel's `keel-<name>` jail-naming or `<pool>/keel/...` dataset-path conventions. Those are `keel-agentd`'s job (a later milestone).
...
**Interfaces:**
- Produces: ZFS datasets `zroot/keel/base/test` (that Tasks 7 and 8's
  integration tests clone from) and `zroot/keel/jails` (the parent
  dataset that `zfs create`/`zfs clone` need to already exist before they
  can create anything underneath it, e.g.
  `zroot/keel/jails/destroy-test-scratch` in Task 7's test — `zfs
...
Run: `! ssh root@192.168.64.2 'zfs create -p zroot/keel/base/test && zfs create zroot/keel/jails && zfs list -r zroot/keel'`

Expected: both datasets are created; `zfs list -r zroot/keel` shows
`zroot/keel`, `zroot/keel/base`, `zroot/keel/base/test`, and
`zroot/keel/jails`.

- [ ] **Step 2: One-time repo clone on the VM**

Run: `! ssh root@192.168.64.2 'git clone git@github.com:LeBaronDeCharlus/keel.git ~/keel 2>&1 || (cd ~/keel && git pull)'`

Expected: the repo clones successfully into `~/keel` on the VM (or, if it
...
`ssh ... 'cd ~/keel && git pull && cargo test ...'` on the VM.

- [ ] **Step 3: Verify the toolchain can build the current tree**

Run: `! ssh root@192.168.64.2 'cd ~/keel && cargo test --workspace 2>&1 | tail -5'`

Expected: the 17 tests from Milestone 1 (`keel-spec`) pass on the VM too
...
### Task 2: keel-jail scaffold — JailError, JailRuntime trait, FakeJailRuntime

**Files:**
- Create: `keel-jail/Cargo.toml`
- Create: `keel-jail/src/error.rs`
- Create: `keel-jail/src/lib.rs`
- Create: `keel-jail/src/fake.rs`
- Modify: `Cargo.toml` (workspace root — add `keel-jail` to `members`)
...
- [ ] **Step 1: Add keel-jail to the workspace**
...
Run: `cargo test --workspace -p keel-jail`
...
Expected: PASS, 22 tests total (17 from `keel-spec` + 5 new).

- [ ] **Step 7: Commit**

```bash
git add Cargo.toml keel-jail/Cargo.toml keel-jail/src/error.rs keel-jail/src/lib.rs keel-jail/src/fake.rs
git commit -m "Add keel-jail crate: JailRuntime trait and FakeJailRuntime"
```
...
**Files:**
- Create: `keel-jail/src/process.rs`
- Modify: `keel-jail/src/lib.rs`
- Create: `keel-jail/tests/freebsd_lifecycle.rs`
...
Modify `keel-jail/src/lib.rs` — add `pub mod process;` and `pub use
...
```bash
git add keel-jail/src/process.rs keel-jail/src/lib.rs keel-jail/tests/freebsd_lifecycle.rs
...
since handles don't survive a `keel-agentd` restart anyway).

Modify `keel-jail/src/process.rs` — replace the whole file:
...
Modify `keel-jail/tests/freebsd_lifecycle.rs` — replace the whole file:
...
```bash
git add keel-jail/src/process.rs keel-jail/tests/freebsd_lifecycle.rs
...
Run: `! ssh root@192.168.64.2 'cd ~/keel && git pull && cargo test -p keel-jail --test freebsd_lifecycle 2>&1 | tail -20'`
...
`mkdir -p /tmp/keel-test-start-command-rootfs/dev && mount -t devfs devfs /tmp/keel-test-start-command-rootfs/dev`
before re-running, and unmount it after
(`umount /tmp/keel-test-start-command-rootfs/dev`). Report back if this
...
In `keel-jail/src/process.rs`, replace the two `unimplemented!()` method
bodies:
...
Add to `keel-jail/tests/freebsd_lifecycle.rs`:
...
new `set_and_remove_resource_limits`. This completes `keel-jail`.

---

### Task 6: keel-zfs scaffold — ZfsError, ZfsManager trait, FakeZfsManager

**Files:**
- Create: `keel-zfs/Cargo.toml`
- Create: `keel-zfs/src/error.rs`
- Create: `keel-zfs/src/lib.rs`
- Create: `keel-zfs/src/fake.rs`
- Modify: `Cargo.toml` (workspace root — add `keel-zfs` to `members`)
...
- [ ] **Step 1: Add keel-zfs to the workspace**
...
[workspace]
resolver = "2"
members = ["keel-spec", "keel-jail", "keel-zfs"]
```
...
Run: `cargo test --workspace -p keel-zfs`
...
```bash
git add Cargo.toml keel-zfs/Cargo.toml keel-zfs/src/error.rs keel-zfs/src/lib.rs keel-zfs/src/fake.rs
git commit -m "Add keel-zfs crate: ZfsManager trait and FakeZfsManager"
```
...
**Files:**
- Create: `keel-zfs/src/cli.rs`
- Modify: `keel-zfs/src/lib.rs`
- Create: `keel-zfs/tests/freebsd_zfs.rs`

**Interfaces:**
- Consumes: `ZfsError`, `ZfsManager` (Task 6), the `zroot/keel/base/test`
...
Modify `keel-zfs/src/lib.rs` — add `pub mod cli;` and `pub use
cli::CliZfsManager;`:
...
```bash
git add keel-zfs/src/cli.rs keel-zfs/src/lib.rs keel-zfs/tests/freebsd_zfs.rs
...
Run: `! ssh root@192.168.64.2 'cd ~/keel && git pull && cargo test -p keel-zfs --test freebsd_zfs 2>&1 | tail -15'`
...
- Modify: `keel-zfs/tests/freebsd_zfs.rs`
...
In `keel-zfs/src/cli.rs`, replace the `unimplemented!()` method body:
...
Add to `keel-zfs/tests/freebsd_zfs.rs`:
...
```bash
git add keel-zfs/src/cli.rs keel-zfs/tests/freebsd_zfs.rs
...
Run: `! ssh root@192.168.64.2 'cd ~/keel && git pull && cargo test -p keel-zfs --test freebsd_zfs 2>&1 | tail -20'`

Expected: all four tests in `freebsd_zfs.rs` pass, including the two new
ones. This completes `keel-zfs` and Milestone 2.

## Milestone Exit Criteria

- `cargo test --workspace` passes with 27 tests on macOS (17 from
  Milestone 1 + 5 `keel-jail` unit tests + 5 `keel-zfs` unit tests), with
  zero FreeBSD dependencies required to run them.
- On the FreeBSD VM: `cargo test -p keel-jail --test freebsd_lifecycle`
  (3 tests) and `cargo test -p keel-zfs --test freebsd_zfs` (4 tests)
  both pass as root.
- `keel-jail::JailRuntime` and `keel-zfs::ZfsManager` are complete,
  fully-implemented traits (no `unimplemented!()` left) ready for
  `keel-agentd`'s reconciliation loop (a later milestone) to compose
