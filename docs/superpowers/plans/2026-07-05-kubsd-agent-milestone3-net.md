# kubsd-agent Milestone 3: kubsd-net Crate — Implementation Plan
...
**Goal:** Build `kubsd-net` — the crate that wires a jail onto a shared
...
`kubsd-jail`'s `create` (shipped in Milestone 2) has no way to create a
VNET-enabled jail, which `kubsd-net` requires.

**Architecture:** Same fake-vs-real trait pattern as `kubsd-jail`/`kubsd-zfs`:
...
`std::process::Command` for shelling out — same as `kubsd-jail`/`kubsd-zfs`.

## Global Constraints

- Design spec: `docs/superpowers/specs/2026-07-05-kubsd-agent-design.md` (Approved). The `NetManager` trait signature and the updated `JailRuntime::create` signature there must match exactly.
...
- No NAT/outbound internet access in `kubsd-net` this milestone — bridge-only L2 connectivity.
- `kubsd-net` takes plain bridge/epair/jail names and addresses it's given — no knowledge of kubsd's naming conventions.
...
so `kubsd-net`'s `attach_jail` (Task 4) needs jails created with
`vnet: true`. This is an early breaking change to `kubsd-jail`'s
...
Modify `kubsd-jail/src/lib.rs` — change the `create` line in the
...
Modify `kubsd-jail/src/fake.rs`. Change the `FakeJail` struct to add a
`vnet` field:
...
Modify `kubsd-jail/src/process.rs` — replace the `create` method:
...
Modify `kubsd-jail/tests/freebsd_lifecycle.rs` — update all four
...
### Task 2: kubsd-net scaffold — NetError, NetManager trait, FakeNetManager

**Files:**
- Create: `kubsd-net/Cargo.toml`
- Create: `kubsd-net/src/error.rs`
- Create: `kubsd-net/src/lib.rs`
- Create: `kubsd-net/src/fake.rs`
- Modify: `Cargo.toml` (workspace root — add `kubsd-net` to `members`)
...
- [ ] **Step 1: Add kubsd-net to the workspace**
...
[workspace]
resolver = "2"
members = ["kubsd-spec", "kubsd-jail", "kubsd-zfs", "kubsd-net"]
```
...
(The `kubsd-jail` dev-dependency is for Task 4's integration test, which
needs a real jail to attach networking to — it is NOT a runtime
dependency of `kubsd-net` itself, which stays FreeBSD-tool-agnostic about
...
mod tests {
    ...
    fn attach_jail_requires_bridge_to_exist_first() {
        let net = FakeNetManager::new();
        assert!(matches!(
            net.attach_jail("web-1", "kubsd0", "epair7", "10.0.0.5/24"),
...
Run: `cargo test --workspace -p kubsd-net`
...
```bash
git add Cargo.toml kubsd-net/Cargo.toml kubsd-net/src/error.rs kubsd-net/src/lib.rs kubsd-net/src/fake.rs
git commit -m "Add kubsd-net crate: NetManager trait and FakeNetManager"
git push origin HEAD
```
...
**Files:**
- Create: `kubsd-net/src/process.rs`
- Modify: `kubsd-net/src/lib.rs`
- Create: `kubsd-net/tests/freebsd_net.rs`
...
kubsd0`) exits 1 with stderr `interface kubsd0 does not exist`. Creating a
...
(`ifconfig bridge0 name kubsd0`) and bringing up (`ifconfig kubsd0 up`)
...
Modify `kubsd-net/src/lib.rs` — add `pub mod process;` and `pub use
...
```bash
git add kubsd-net/src/process.rs kubsd-net/src/lib.rs kubsd-net/tests/freebsd_net.rs
...
Run: `ssh root@192.168.64.2 'cd ~/kubsd && git pull && cargo test -p kubsd-net --test freebsd_net 2>&1 | tail -15'`
...
**Files:**
- Modify: `kubsd-net/src/process.rs`
- Modify: `kubsd-net/tests/freebsd_net.rs`

**Interfaces:**
- Consumes: `NetError`, `NetManager`, `ProcessNetManager` (Task 3);
  `kubsd_jail::{JailRuntime, ProcessJailRuntime}` (Milestone 2, with
...
In `kubsd-net/src/process.rs`, replace the `unimplemented!()` body:
...
Add to `kubsd-net/tests/freebsd_net.rs`:
...
```bash
git add kubsd-net/src/process.rs kubsd-net/tests/freebsd_net.rs kubsd-net/Cargo.toml
...
Run: `ssh root@192.168.64.2 'cd ~/kubsd && git pull && cargo test -p kubsd-net --test freebsd_net 2>&1 | tail -20'`
...
```bash
git add kubsd-net/src/process.rs kubsd-net/tests/freebsd_net.rs
...
Run: `ssh root@192.168.64.2 'cd ~/kubsd && git pull && cargo test -p kubsd-net --test freebsd_net 2>&1 | tail -25'`

Expected: all five tests in `freebsd_net.rs` pass. This completes
`kubsd-net` and Milestone 3.

## Milestone Exit Criteria

- `cargo test --workspace` passes with 33 tests on macOS (28 from before
  this milestone + 5 `kubsd-net` unit tests), with zero FreeBSD
  dependencies required to run them.
- On the FreeBSD VM: `cargo test -p kubsd-jail --test freebsd_lifecycle`
  (4 tests, using the new `vnet` parameter), and `cargo test -p kubsd-net
  --test freebsd_net` (5 tests) both pass as root.
- `kubsd-net::NetManager` is a complete, fully-implemented trait (no
  `unimplemented!()` left) ready for `kubsd-agentd`'s reconciliation loop
  (a later milestone) to compose against a fake in its own unit tests,
  alongside `kubsd-jail` and `kubsd-zfs`.
