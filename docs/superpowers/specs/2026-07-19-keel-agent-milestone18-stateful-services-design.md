...
- **No operator "force re-pin" or "force reschedule" verb.** If a pinned node is genuinely gone for good, there is no way in this milestone to manually evict that one replica and let the scheduler place a fresh one elsewhere: `keelctl delete <replica-name>`, routed through the scheduled-delete path, never even reaches the forward step. `handle_scheduled_delete` resolves the placement's node via `registry.resolve` first; for a `Dead` node that fails and `resolve_placement` returns `404` immediately, the same response an unplaced name would get, since a placement is only cleared after a confirmed successful remote delete. This is an accepted, bounded gap, the same shape as Milestone 17's own accepted "delete-then-recreate onto a different node silently starts empty" gap; a later milestone could add an explicit force-unpin verb if this turns out to matter in practice.
...
- Manually deleting one stateful replica by name while its pinned node is unreachable surfaces the ordinary `404` that `resolve_placement` already gives for any unreachable node, indistinguishable from the replica never having been placed at all: this milestone adds no special handling for that case, per Non-Goals above.
