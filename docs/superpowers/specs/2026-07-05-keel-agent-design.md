# keel-agent: Single-Node FreeBSD Jail Reconciliation Daemon
...
keel is a long-term effort to build a Kubernetes-style orchestration platform
...
one pass. This spec covers only the first sub-project: **keel-agent**, the
...
- Expose a local HTTP API (over a Unix socket) and a companion CLI
  (`keelctl`) to apply/get/delete specs.
...
- **`keel-spec`** — the jail spec schema (YAML), (de)serialization, and
  validation. Pure Rust; compiles and tests on any OS.
- **`keel-jail`** — wraps jail lifecycle and resource-limit management
  ...
  first (same escape hatch as `keel-zfs`) gets a working v1 sooner with
  ...
  into the same `false`, and `keel-agentd`'s reconciler needs to tell
  those apart: the former needs full provisioning, the latter just needs
  `start_command` again.

  `keel-jail` takes a plain jail name string — it has no knowledge of
  keel's `keel-<name>` naming/ownership convention; applying that prefix
  is `keel-agentd`'s responsibility. `create`'s `vnet` parameter controls
  whether the jail gets `jail -c ... vnet persist` (its own network stack,
  required before `keel-net`'s `attach_jail` can move an interface into
  it — VNET can only be set at jail-creation time, not added
  retroactively) or plain `jail -c ... persist` (shares the host's network
  stack, e.g. for a jail with no `keel-net` wiring). `create` and
  ...
  `keel-agentd` can configure networking (via `keel-net`) and apply
  ...
  on it) and is also the primitive `keel-agentd` re-invokes on every
  ...
  handle wouldn't survive a `keel-agentd` restart and would break
...
- **`keel-zfs`** — wraps ZFS dataset/snapshot/clone operations used to
  ...
  Like `keel-jail`, this crate takes full dataset path strings and has no
  knowledge of keel's path convention (`<pool>/keel/base/<image>`,
  `<pool>/keel/jails/<name>`) — that's `keel-agentd`'s job. `clone_from_base`
  can't clone a live dataset directly, so it ensures a canonical snapshot
  `<base_dataset>@keel` exists (creating it on demand if missing — the
...
- **`keel-net`** — sets up `epair(4)` + `bridge(4)` + VNET wiring per jail.
  ...
  actually starting), the same way `keel-zfs`'s `clone_from_base`
  ...
  success rather than an error, mirroring `keel-jail`'s
  `remove_resource_limits`. Like `keel-jail` and `keel-zfs`, this crate
  takes plain bridge/epair/jail names it's given — it has no knowledge of
  keel's `keel0` bridge-naming or per-jail epair-ordinal conventions;
  that's `keel-agentd`'s job.
- **`keel-agentd`** — the daemon binary. Hosts the local HTTP API and owns
  the reconciliation loop; composes the traits above.
- **`keelctl`** — CLI binary that talks to `keel-agentd`'s API to
...
decision: it lets the reconciliation logic in `keel-agentd` be fully unit
...
`keel-spec` owns parsing and validation of a single spec document
...
single-document parser — it's `keel-agentd`'s state-store concern at
apply time, not `keel-spec`'s.
...
**Base images.** Populating `<pool>/keel/base/<image-name>` (e.g. via
`bsdinstall` into a dataset, or `zfs recv` of a prepared image) is out of
scope for keel-agent v1 — base datasets are prepared out-of-band by the
...
either is rejected by `keel-spec` validation with an error telling the
...
- The FreeBSD jail itself is named `keel-<jail-name>` (the actual
  ...
  considers jails whose name has the `keel-` prefix as agent-managed;
...
- Each jail's ZFS clone lives at a fixed path derived from its name:
  `<pool>/keel/jails/<jail-name>`. Base images live at
  `<pool>/keel/<image>` — the spec's `image` field already includes its
  full relative path under `keel/` (e.g. `image: base/14.2-web` maps to
  `<pool>/keel/base/14.2-web`, not `<pool>/keel/base/base/14.2-web`).
  The jail's rootfs filesystem path passed to `keel-jail`'s `create` is
  ...
  Milestone 2 (`zroot/keel/base/test` mounts at
  `/zroot/keel/base/test`).

State is persisted under `/var/db/keel/`: last-applied specs (plus the
...
1. Maintain **desired state** (from applied specs, persisted to disk) and
   **observed state** (queried live via `keel-jail`/`rctl` on the running
   system).
2. On a timer (e.g. every 5s) and immediately after any API-triggered
   ...
   - Desired but not existing → provision rootfs (ZFS clone from base
     image), create the jail (`keel-jail`'s `create`), configure
     networking (epair/bridge/VNET), apply `rctl` limits, then start the
     command (`keel-jail`'s `start_command`).
   - Existing but not desired → remove network interfaces (`keel-net`'s
     `detach_jail`), destroy the jail (`keel-jail`'s `destroy`, which
     ...
     `stop_gracefully` primitive to `keel-jail` is a future enhancement,
     ...
     `keel-agentd` Implementation below, and the corresponding Open
   ...
   - Existing and desired but the process has exited → apply
     ...
     behave identically (both restart on any exit) — `keel-jail` has no
...
5. The reconciler is single-threaded. **Milestone 4 simplification:** the
   ...
   `keel-agentd` Implementation below for the concrete API shape.

## keel-agentd Implementation (Milestone 4)

`keel-agentd` is a **library crate only** for this milestone — no
...
`keel-spec`, `keel-jail`, `keel-zfs`, and `keel-net`, so tests
...
**Files:** `keel-agentd/src/record.rs` (`JailRecord`, naming/path
derivation), `keel-agentd/src/store.rs` (persistence),
`keel-agentd/src/backoff.rs` (`BackoffState`),
`keel-agentd/src/reconciler.rs` (the `Reconciler` itself).

**Data model:**

```rust
pub struct JailRecord {
    pub spec: JailSpec,       // from keel-spec
    pub epair_ordinal: u32,   // assigned once at first apply, stable across restarts
}
```

Naming/path derivation (pure functions):
- Jail name: `keel-<spec.metadata.name>`
- Base dataset: `<pool>/keel/<spec.spec.image>`
- Jail's own dataset: `<pool>/keel/jails/<spec.metadata.name>`
- Jail rootfs path (passed to `keel-jail`'s `create`): `/<jail-dataset-path>`
...
- `apply`: validates the spec (`keel-spec`'s existing checks, plus
...
**Backoff** (`keel-agentd/src/backoff.rs`):
...
is intrinsically idempotent on all implementations (built into `keel-net`
...
- Partial failure during jail creation (e.g. ZFS clone succeeds but
  `keel-jail`'s `create` fails) triggers automatic cleanup of the
...
- **Jails outlive the daemon.** Jails are not children of `keel-agentd`
  ...
  above (the `keel-` name prefix) necessary: on restart the daemon must
...
- **Unix socket trust model (v1):** the API socket is `root:wheel 0600`.
  ...
  oversight.
- **Daemon supervision:** `keel-agentd` runs under an `rc.d` script with
...
- `keel-spec`: plain unit tests, run in CI on any OS.
- `keel-agentd` reconciliation logic: unit tested against fake
...
- `keel-jail`, `keel-zfs`, `keel-net`: integration tests that exercise
  ...
  VM (`git@github.com:LeBaronDeCharlus/keel.git`), then `git pull` +
  ...
  that access is ever unavailable again). A `zroot/keel/base/test`
  dataset (a minimal FreeBSD userland) and a `zroot/keel/jails` parent
  dataset must exist on the VM for `keel-zfs`'s clone tests — a one-time
  VM setup step, same category as Milestone 1's environment prep.
  `keel-net`'s integration tests additionally need a real jail to attach
  to (created via `keel-jail`) and must explicitly tear down any
  leftover bridge/epair interfaces from a failed prior run, since
  `ensure_bridge_exists` never removes bridges.
- End-to-end tests (apply a spec via `keelctl`, assert the jail is actually
...
- Whether `keel-spec` should validate the `apiVersion`/`kind` discriminant
...
- Whether `keel-jail` moves from shelling out to `jail(8)`/`rctl(8)` to
  raw `jail_set(2)`/`rctl` syscalls, and whether `keel-zfs` moves from
...
- ~~Whether the Reconciliation Loop's "existing and desired but the
  ...
  **Resolved in Milestone 4:** yes, it always reapplies both (idempotently)
  before restarting the command. See `keel-agentd` Implementation above.
...
- Graceful jailed-process shutdown (`SIGTERM`, wait up to a grace period,
  ...
  deferred; `keel-jail`'s existing `destroy` (`jail -r`) is used as-is for
  v1. Would need a new `keel-jail` primitive (e.g. `stop_gracefully`) if
  ever implemented.
- Distinguishing `restartPolicy: OnFailure` from `Always` — currently
  identical behavior (both restart on any exit), since `keel-jail` has no
