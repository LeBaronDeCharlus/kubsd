...
- `keel-controlplane` gains a new `--service-cidr` flag (a cluster-wide
  ...
  deterministically from the service name the same way Milestone 14's
  `derive_pod_cidr` already derives a node's `pod_cidr` from its node id,
...
- Every node's `keel-agentd`, on each heartbeat round-trip, keeps its own
  ...
  `keel0` bridge (the same aliasing trick Milestone 14 already uses for
  the bridge's own gateway address), and a TCP relay listener bound to
...
service's identity), and `port: u16`, carried straight through from the
applied spec. `Command::ApplyService` gains `port: u16` as a new
parameter (alongside the existing name/replicas/template), and
`handle_apply_service` (`http.rs`) passes `spec.spec.port` through to it.

VIP derivation mirrors Milestone 14's `derive_pod_cidr` exactly: hash the
service name into an address within `service_cidr`, and if that candidate
is already assigned to a different service, linearly probe forward
(bounded by the CIDR's actual size) until a free one is found or the pool
is exhausted (a hard `apply`-time error in the latter case — see Error
Handling). This is a deliberate reuse of an already-reviewed pattern, not
a new design.
...
implemented via `ifconfig <bridge> alias`/`-alias`, the same underlying
mechanism Milestone 14's `bridge_gateway` already uses to give a bridge its
own gateway address — generalized here to accept an arbitrary address
instead of one hardcoded one. `FakeNetManager` gains an in-memory
equivalent.
...
10.0.250.0/24` → Milestone 15's existing reconciliation places the two
...
- VIP allocation is a hard requirement, unlike replica placement (which
  ...
  linear probing, the same as `pod_cidr`'s existing collision handling;
  exhausting `service_cidr` entirely is an outright `apply` rejection, not
...
- A node that can't refresh (heartbeat failing, e.g. the control plane is
  ...
  self-healing-over-durability stance since Milestone 4.
...
- **VM verification (three real nodes, same discipline as every
  milestone):** apply a 2-replica service, confirm the VIP is aliased on
  all three nodes' bridges; connect to `<vip>:<port>` from a jail and
  confirm it reaches a replica, repeated enough times to confirm both
  replicas actually get used; kill one replica's node and confirm the
  survivor keeps answering after the next reconcile; delete the service
  and confirm the VIP alias disappears from every node and new
